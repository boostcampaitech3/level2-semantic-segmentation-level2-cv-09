...
def train(data_dir, model_dir, args):
    ...
    train_path = data_dir + "/stratified_kfold/train2.json"
    val_path = data_dir + "/stratified_kfold/val2.json"
    ...
    # -- model
    config_path = "/opt/ml/input/code/model/hcrV2_ocr/hrnet_w64_seg_ocr.yaml" # 경로 수정!
    ...
    scheduler = CosineAnnealingWarmUpRestarts(
        optimizer, T_0=1600, T_mult=2, eta_max=4e-5, T_up=800, gamma=0.5
    )
    class_labels = {
        0: "Backgroud",
        1: "General trash",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            if mIoU > best_mIoU:
                ...
                torch.save(
                    model.state_dict(),
                    f"{save_dir}/{args.name}_HRNetV2_W64_OCR_{epoch}_{round(mIoU,4)}.pth",
                )
                best_mIoU = mIoU
                er_cnt=0
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epochs", type=int, default=200, help="number of epochs to train (default: 25)"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--name", default="54-3_", help="model save at {SM_MODEL_DIR}/{name}"
    )
    ...
    wandb.init(name="hrnetv2_w64_ocr",project="seg-baseline", entity="level2-cv-09")
    wandb.run.name = (
        f"hrnetv2_w64_ocr_aug"
    )
